---
title: "SharedKernel Persistent Subscription to $all"
sidebarTitle: "Persistent Subscription"
description: "KurrentDbAllStreamPersistentSubscription implements a persistent subscription to the $all stream of KurrentDB using the native consumer group pattern."
---
...
<Warning>
  `MaxRetryCount` and `FilterOptions` only take effect during group **creation**. If the group already exists in KurrentDB, modifying these options will not update it. To change them on an existing group, it must be deleted and recreated.
</Warning>
