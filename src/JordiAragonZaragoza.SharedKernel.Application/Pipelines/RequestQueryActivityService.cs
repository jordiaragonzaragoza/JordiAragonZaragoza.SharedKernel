namespace JordiAragonZaragoza.SharedKernel.Application.Pipelines
{
    ...
    public class RequestQueryActivityService<TRequest> : IRequestQueryActivityService<TRequest>
        where TRequest : notnull
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            using var activity = ActivitySource.StartActivity(
                $"QueryHandler: {typeof(TRequest).Name}",
                ActivityKind.Internal);

            activity?.SetTag("query.type", typeof(TRequest).FullName);
